...
import type { MovieListEntry, ScheduleListEntry, Screen } from "@/domains";
...
export default function ScheduleSelectSubpage({ className }: BaseProps) {
  ...
  const { selectedDate, selectedMovies, selectedSchedule, updateSelectedSchedule } = useTicketingStore();
  ...
  const schedules = useGetApi(
    () =>
      getSchedules({
        page: 0,
        size: 99,
        movieNum: selectedMovies.at(0)?.movieNum,
        date: selectedDate,
